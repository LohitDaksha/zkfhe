...
bool matches_dim(const ConstCiphertext<DCRTPoly>& ciphertext, const size_t num_polys, const size_t num_limbs,
...
void NTTParameters(const DCRTPoly::PolyType::Vector::Integer rootOfUnity, const usint CycloOrder,
                   const DCRTPoly::PolyType::Integer modulus, DCRTPoly::PolyType::Vector& rootOfUnityTable,
                   DCRTPoly::PolyType::Vector& preconRootOfUnityTable) {
    const usint CycloOrderHf = (CycloOrder >> 1);

    using CRT      = intnat::ChineseRemainderTransformFTTNat<DCRTPoly::PolyType::Vector>;
    ...
    rootOfUnityTable       = CRT::m_rootOfUnityReverseTableByModulus[modulus];
    preconRootOfUnityTable = CRT::m_rootOfUnityPreconReverseTableByModulus[modulus];
}
void INTTParameters(const DCRTPoly::PolyType::Vector::Integer rootOfUnity, const usint CycloOrder,
                    const DCRTPoly::PolyType::Integer modulus, DCRTPoly::PolyType::Vector& rootOfUnityInverseTable,
    ...
    const usint CycloOrderHf = (CycloOrder >> 1);
    const usint msb          = lbcrypto::GetMSB64(CycloOrderHf - 1);
    using CRT                = intnat::ChineseRemainderTransformFTTNat<DCRTPoly::PolyType::Vector>;
    ...
    rootOfUnityInverseTable       = CRT::m_rootOfUnityInverseReverseTableByModulus[modulus];
    preconRootOfUnityInverseTable = CRT::m_rootOfUnityInversePreconReverseTableByModulus[modulus];
    cycloOrderInv                 = CRT::m_cycloOrderInverseTableByModulus[modulus][msb];
    preconCycloOrderInv           = CRT::m_cycloOrderInversePreconTableByModulus[modulus][msb];
}
...
[[nodiscard]] vector<std::shared_ptr<gadget_gen<FieldT>>> LibsnarkProofSystem::constrain_addmod_lazy(
...
LibsnarkConstraintMetadata LibsnarkProofSystem::PublicInputConstraint(ConstCiphertext<DCRTPoly> ciphertext) {
    ...
    assert(out.matches_dim(ciphertext));
    return out;
}

void LibsnarkProofSystem::PublicInputWitness(ConstCiphertext<DCRTPoly> ciphertext) {
    const auto witnessMetadata = GetMetadata<LibsnarkWitnessMetadata>(ciphertext);
    ...
    for (size_t i = 0; i < num_polys; i++) {
        ...
        for (size_t j = 0; j < num_limbs; j++) {
            ...
            for (size_t k = 0; k < num_coeffs; k++) {
                ...
                pb.val(pb_variable<FieldT>(curr_index)) = FieldT(v_ij[k].ConvertToInt<long>());
            }
        }
    }
}
...
void LibsnarkProofSystem::EvalAddWitness(ConstCiphertext<DCRTPoly> ctxt1, ConstCiphertext<DCRTPoly> ctxt2,
    ...
    const auto witnessMetadata = GetMetadata<LibsnarkWitnessMetadata>(ctxt_out);
...
//    const size_t ptxt_modulus = pt.GetElementAtIndex(0).GetModulus().ConvertToInt<long>();
...
//            pb.val(var) = FieldT(pt.GetElementAtIndex(0).GetValues()[j].ConvertToInt<long>());
//
...
LibsnarkConstraintMetadata LibsnarkProofSystem::EvalSubConstraint(const LibsnarkConstraintMetadata& in1,
    ...
    //        out[i].resize(num_limbs);
    //        out.max_value[i].resize(num_limbs);
    //        constrain_submod_lazy(in1, i, in2, i, out, i, witnessMetadata.gadgets);
    //    }
    out.witness_metadata = std::move(witnessMetadata);
    return out;
}
...
//
//void LibsnarkProofSystem::ConstrainSubtraction(ConstCiphertext<DCRTPoly> ctxt, const Plaintext& ptxt,
//                                                Ciphertext<DCRTPoly>& ctxt_out) {
//    const auto in1 = *GetProofMetadata(ctxt);
//    auto pt        = ptxt->GetElement<DCRTPoly>();
//    assert(in1[0].size() == pt.GetNumOfElements());
...
LibsnarkConstraintMetadata LibsnarkProofSystem::EvalMultNoRelinConstraint(const LibsnarkConstraintMetadata& in1,
    ...
    LibsnarkConstraintMetadata tmp(2);
    tmp.modulus = in1.modulus;
    for (size_t i = 0; i < tmp.size(); i++) {
        tmp[i].resize(num_limbs);
        for (size_t j = 0; j < num_limbs; j++) {
            tmp[i][j].resize(num_coeffs);
        }
        tmp.max_value[i].resize(num_limbs);
    }

    LibsnarkWitnessMetadata witnessMetadata;

    constrain_mulmod_lazy(in1, 0, in2, 0, out, 0, witnessMetadata.gadgets);

    constrain_mulmod_lazy(in1, 0, in2, 1, tmp, 0, witnessMetadata.gadgets);
    constrain_mulmod_lazy(in1, 1, in2, 0, tmp, 1, witnessMetadata.gadgets);
    constrain_addmod_lazy(tmp, 0, tmp, 1, out, 1, witnessMetadata.gadgets);

    constrain_mulmod_lazy(in1, 1, in2, 1, out, 2, witnessMetadata.gadgets);
...
void LibsnarkProofSystem::EvalSquareWitness(ConstCiphertext<DCRTPoly> ciphertext,
    ...
    const auto witnessMetadata = GetMetadata<LibsnarkWitnessMetadata>(ciphertext_out);
    for (const auto& g : witnessMetadata.gadgets) {
        g->generate_r1cs_witness();
    }
}
...
LibsnarkConstraintMetadata LibsnarkProofSystem::RescaleConstraint(ConstCiphertext<DCRTPoly> ctxt_in,
    ...
    for (size_t i = 0; i < num_polys; ++i) {
        ...
        SetFormatConstraint(Format::COEFFICIENT, lastPoly, lastPolyCoef, m[i][num_limbs - 1],
                            m.max_value[i][num_limbs - 1], last_lc, last_max_value, out.witness_metadata);
        ...
        for (usint j = 0; j < extra.GetNumOfElements(); j++) {
            ...
            SwitchModulusConstraint(newMod, newRoU, lastPoly, temp, last_lc, last_max_value, extra_lc[j],
                                    extra_max_value[j], out.witness_metadata);
        ...
        SetFormatConstraint(Format::EVALUATION, extra, extra_eval, extra_lc, extra_max_value, extra_eval_lc,
                            extra_eval_max_value, out.witness_metadata);
        ...
        for (size_t j = 0; j < out[i].size(); j++) {
            ...
            for (size_t k = 0; k < out[i][j].size(); k++) {
                ...
                out.witness_metadata.add_gadget(g);
            }
...
LibsnarkConstraintMetadata LibsnarkProofSystem::EvalRotateConstraint(ConstCiphertext<DCRTPoly> ciphertext,
                                                                     const int rot_idx,
                                                                     ConstCiphertext<DCRTPoly> ctxt_out) {
    const auto in = GetMetadata<LibsnarkConstraintMetadata>(ciphertext);
...
LibsnarkConstraintMetadata LibsnarkProofSystem::RelinearizeConstraint(ConstCiphertext<DCRTPoly> ciphertext) {
    ...
    for (size_t j = 2; j < num_poly; j++) {
        ...
        ConstrainKeySwitchPrecomputeCore(cv[j], evalKey->GetCryptoParameters(), digits, in[j], in.max_value[j], tmp_lc,
                                         tmp_max_value, out_metadata.witness_metadata);
        ...
        constrain_addmod_lazy(out_metadata, 0, tmp_metadata, 0, out_metadata, 0, out_metadata.witness_metadata.gadgets);
        constrain_addmod_lazy(out_metadata, 1, tmp_metadata, 1, out_metadata, 1, out_metadata.witness_metadata.gadgets);
    }
...
LibsnarkConstraintMetadata LibsnarkProofSystem::KeySwitchConstraint(ConstCiphertext<DCRTPoly> ctxt_in,
    ...
    const auto in = GetMetadata<LibsnarkConstraintMetadata>(ctxt_in);
    ...
    for (size_t j = 0; j < out[0].size(); j++) {
        for (size_t k = 0; k < out[0][j].size(); k++) {
            LazyAddModGadget<FieldT> g(pb, out[0][j][k], out.max_value[0][j], in[0][j][k], in.max_value[0][j],
                                       in.modulus[j]);
            g.generate_r1cs_constraints();
            // TODO: g.out is not used anywhere?
...
std::shared_ptr<LibsnarkConstraintMetadata> LibsnarkProofSystem::ConstrainPublicOutput(
    ...
    for (size_t j = 0; j < num_limbs; j++) {
        out.modulus[j] = ciphertext->GetElements()[0].GetElementAtIndex(j).GetModulus().ConvertToInt<long>();
    }
...
void LibsnarkProofSystem::FinalizeOutputConstraints(Ciphertext<DCRTPoly>& ctxt,
    ...
    auto out           = GetMetadata<LibsnarkConstraintMetadata>(ctxt);
...
void LibsnarkProofSystem::SwitchModulusConstraint(const DCRTPoly::PolyType::Vector::Integer& newModulus,
                                                  const DCRTPoly::PolyType::Vector::Integer& rootOfUnity,
                                                  const DCRTPoly::PolyType& in, const DCRTPoly::PolyType& out,
                                                  const vector<pb_linear_combination<FieldT>>& in_lc,
                                                  const FieldT& in_max_value,
                                                  vector<pb_linear_combination<FieldT>>& out_lc, FieldT& out_max_value,
                                                  LibsnarkWitnessMetadata& witness_metadata) {
    assert(newModulus != in.GetModulus());
 ...
 */
    ...
    for (int i = 0; i < out_lc.size(); i++) {
        ...
        assert(mod(pb.lc_val(in_lc[i]), FieldT(in.GetModulus().template ConvertToInt<long>())) ==
               FieldT(in[i].template ConvertToInt<long>()));
    }
#endif
    out_lc.resize(in_lc.size());

    const auto& oldModulus(in.GetModulus());
    ...
    auto old_mod_int      = oldModulus.template ConvertToInt<long>();
    if (gt_eq(in_max_value, FieldT(old_mod_int))) {
        for (usint i = 0; i < in.GetLength(); i++) {
            ...
            witness_metadata.add_gadget(std::move(g));
        }
    ...
    if (newModulus > oldModulus) {
        for (usint i = 0; i < in.GetLength(); i++) {
            ...
            is_less_than_constant_gadget<FieldT> g(pb, in_red_lc[i], in_red_max_value.as_bigint().num_bits(),
                                                   FieldT(oldModulusByTwo.template ConvertToInt<long>()) + 1);
            g.generate_r1cs_constraints();
            out_lc[i].assign(pb, in_red_lc[i] + (1 - g.less_or_eq) * FieldT(diff.template ConvertToInt<long>()));

            witness_metadata.add_gadget(g);
        }
    }
    else {
        ...
        for (usint i = 0; i < in.GetLength(); i++) {
            ...
            tmp.assign(pb, in_red_lc[i] - (1 - g.less_or_eq) * FieldT(diff.template ConvertToInt<long>()));
            FieldT tmp_max_value =
                FieldT(std::max(oldModulusByTwo, oldModulus - 1 - diff).template ConvertToInt<long>());
            ...
            ModGadget<FieldT> g_mod(pb, tmp, tmp_max_value, newModulus.template ConvertToInt<long>(), "", false);
            out_lc[i] = g_mod.out;
            g_mod.generate_r1cs_constraints();

            witness_metadata.add_gadget(g_mod);
        }
    }
    out_max_value = newModulus.template ConvertToInt<long>() - 1;
#ifdef PROOFSYSTEM_CHECK_STRICT
    for (const auto& g : witness_metadata.gadgets) {
        g->generate_r1cs_witness();
    }
    for (int i = 0; i < out_lc.size(); i++) {
        out_lc[i].evaluate(pb);
        assert(lt_eq(pb.lc_val(out_lc[i]), out_max_value));
        assert(pb.lc_val(out_lc[i]) == FieldT(out[i].template ConvertToInt<long>()));
    }
#endif
}

void LibsnarkProofSystem::SwitchModulusWitness(LibsnarkWitnessMetadata& witness_metadata) {
    for (const auto& g : witness_metadata.gadgets) {
        g->generate_r1cs_witness();
    }
}

void LibsnarkProofSystem::NTTOpenfheConstraint(
    const DCRTPoly::PolyType::Vector& rootOfUnityTable, const DCRTPoly::PolyType::Vector& preconRootOfUnityTable,
    ...
    vector<pb_linear_combination<FieldT>>& out_lc, FieldT& out_max_value, LibsnarkWitnessMetadata& witness_metadata) {
    ...
    for (int i = 0; i < in_lc.size(); i++) {
        in_lc[i].evaluate(pb);
        assert(lt_eq(pb.lc_val(in_lc[i]), in_max_value));
        assert(mod(pb.lc_val(in_lc[i]), FieldT(element.GetModulus().template ConvertToInt<long>())) ==
               FieldT(element[i].template ConvertToInt<long>()));
    }
#endif
    ...
    unsigned long q                             = modulus.template ConvertToInt<long>();
    ...
    for (uint32_t m = 1; m < n; m <<= 1, t >>= 1, --logt1) {
        ...
        for (uint32_t i = 0; i < m; ++i) {
            ...
            for (uint32_t indexLo = j1; indexLo < j2; ++indexLo) {
                ...
                witness_metadata.add_gadget(std::move(g1));
                witness_metadata.add_gadget(std::move(g2));
                witness_metadata.add_gadget(std::move(g3));
    ...
    for (int i = 0; i < out_lc.size(); i++) {
        out_lc[i].evaluate(pb);
        assert(lt_eq(pb.lc_val(out_lc[i]), out_max_value));
        assert(mod(pb.lc_val(out_lc[i]), FieldT(q)) == FieldT(element_out[i].template ConvertToInt<long>()));
    }
#endif
}

void LibsnarkProofSystem::NTTOpenfheWitness(LibsnarkWitnessMetadata& witness_metadata) {
    for (const auto& g : witness_metadata.gadgets) {
        g->generate_r1cs_witness();
    }
}

void LibsnarkProofSystem::NTTLinalgConstraint(const DCRTPoly::PolyType& element_in,
    ...
    const auto q_int = q.template ConvertToInt<long>();
    ...
    for (size_t i = 0; i < n; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            pows_int[i][j]          = pows[i][j].template ConvertToInt<long>();
        }
    }
...
#pragma omp parallel for default(none) shared(n, lc, ntt_in_lc, pows_int)
    ...
    for (int i = 0; i < out_lc.size(); i++) {
        out_lc[i].evaluate(pb);
        assert(lt_eq(pb.lc_val(out_lc[i]), out_max_value));
        assert(mod(pb.lc_val(out_lc[i]), FieldT(q.ConvertToInt<long>())) ==
               FieldT(element_out[i].ConvertToInt<long>()));
    }
#endif
}

void LibsnarkProofSystem::NTTLinalgWitness(LibsnarkWitnessMetadata& witness_metadata) {
    for (const auto& g : witness_metadata.gadgets) {
        g->generate_r1cs_witness();
    }
}

void LibsnarkProofSystem::INTTOpenfheConstraint(const DCRTPoly::PolyType::Vector& rootOfUnityInverseTable,
        ...
        for (i = 0; i < m; ++i) {
            ...
            for (indexLo = j1; indexLo < j2; ++indexLo) {
                ...
                if (lt_eq(out_max_values[indexHi], FieldT(q))) {
                    ...
                    witness_metadata.add_gadget<ModGadget<FieldT>>(std::move(g_mod));
                }

                LazySubModGadget<FieldT> g2(this->pb, out_lc[indexLo], out_max_values[indexLo], hi_reduced,
                                            hi_reduced_max_value, q);
                LazyMulModGadget<FieldT> g3(this->pb, g2.out, g2.out_max_value, FieldT(omega.ConvertToInt<long>()), q);
                ...
                witness_metadata.add_gadget<LazyAddModGadget<FieldT>>(std::move(g1));
                witness_metadata.add_gadget<LazySubModGadget<FieldT>>(std::move(g2));
                witness_metadata.add_gadget<LazyMulModGadget<FieldT>>(std::move(g3));
    ...
    for (i = 0; i < n; i++) {
        ...
        LazyMulModGadget<FieldT> g(this->pb, out_lc[i], out_max_values[i], FieldT(cycloOrderInv.ConvertToInt<long>()),
                                   q);
        g.generate_r1cs_constraints();
        g.generate_r1cs_witness();
        out_lc[i]         = g.out;
...
void LibsnarkProofSystem::INTTOpenfheWitness(LibsnarkWitnessMetadata& witness_metadata) {
    for (const auto& g : witness_metadata.gadgets) {
        g->generate_r1cs_witness();
    }
}

void LibsnarkProofSystem::SetFormatConstraint(const Format format, const DCRTPoly::PolyType& in,
    ...
    if (format == COEFFICIENT) {
#if defined(PROOFSYSTEM_R1CS_NTT_OPENFHE)
        ...
        INTTOpenfheConstraint(rootOfUnityInverseTable, preconRootOfUnityInverseTable, cycloOrderInv,
                              preconCycloOrderInv, in, out, in_lc, in_max_value, out_lc, out_max_value,
                              witness_metadata);
#else
        static_assert(false && "Either 'PROOFSYSTEM_R1CS_NTT_LINALG' or 'PROOFSYSTEM_R1CS_NTT_OPENFHE' must be set");
#endif
    }
    else {
#ifdef PROOFSYSTEM_R1CS_NTT_OPENFHE
...
void LibsnarkProofSystem::SetFormatConstraint(const Format format, const DCRTPoly& in, const DCRTPoly& out,
    ...
    for (size_t i = 0; i < n; i++) {
        SetFormatConstraint(format, in.GetElementAtIndex(i), out.GetElementAtIndex(i), in_lc[i], in_max_value[i],
                            out_lc[i], out_max_value[i], witness_metadata);
    }
}

void LibsnarkProofSystem::SetFormatWitness(const Format format, LibsnarkWitnessMetadata& witness_metadata) {
    if (format == COEFFICIENT) {
#if defined(PROOFSYSTEM_R1CS_NTT_OPENFHE)
        INTTOpenfheWitness(witness_metadata);
#elif defined(PROOFSYSTEM_R1CS_NTT_LINALG)
        // TODO: implement linalg variant for INTT as well
        INTTOpenfheWitness(witness_metadata);
#else
...
#ifdef PROOFSYSTEM_R1CS_NTT_OPENFHE
        NTTOpenfheWitness(witness_metadata);
#elif defined(PROOFSYSTEM_R1CS_NTT_LINALG)
        NTTLinalgWitness(witness_metadata);
#else
        static_assert(false && "Either 'PROOFSYSTEM_R1CS_NTT_LINALG' or 'PROOFSYSTEM_R1CS_NTT_OPENFHE' must be set");
#endif
    }
...
void LibsnarkProofSystem::ConstrainKeySwitchPrecomputeCore(
    ...
    SetFormatConstraint(Format::COEFFICIENT, in, input, in_lc, in_max_value, in_coeff_lc, in_coeff_max_value,
                        witness_metadata);
        ...
        if (baseBits == 0) {
            ...
            for (usint k = 0; k < num_levels; k++) {
                ...
                if (i != k) {
                    ...
                    SwitchModulusConstraint(input.GetElementAtIndex(k).GetModulus(),
                                            input.GetElementAtIndex(k).GetRootOfUnity(), old_temp, temp, old_temp_lc,
                                            old_temp_max_value, temp_lc, temp_max_value, witness_metadata);

                    // temp.SetFormat(Format::EVALUATION);
                    SetFormatConstraint(Format::EVALUATION, temp, (*out)[i].GetElementAtIndex(k), temp_lc,
                                        temp_max_value, out_lc[i][k], out_max_value[i][k], witness_metadata);
                }
                    ...
                    SetFormatConstraint(Format::EVALUATION, input.GetElementAtIndex(k), (*out)[i].GetElementAtIndex(k),
...
void LibsnarkProofSystem::ConstrainFastKeySwitchCore(const EvalKey<DCRTPoly>& evalKey,
    ...
    for (size_t j = 0; j < n; ++j) {
        ...
        size_t modulus = av[0].GetElementAtIndex(j).GetModulus().ConvertToInt<long>();
        for (size_t k = 0; k < in_lc[0][j].size(); ++k) {
            auto av_0jk = av[0].GetElementAtIndex(j).GetValues()[k];
            LazyMulModGadget<FieldT> g1(pb, in_lc[0][j][k], in_max_value[0][j], FieldT(av_0jk.ConvertToInt<long>()),
                                        modulus);
            g1.generate_r1cs_constraints();
            g1.generate_r1cs_witness();
            out_lc[1][j][k]       = g1.out;
            ...
            LazyMulModGadget<FieldT> g0(pb, in_lc[0][j][k], in_max_value[0][j], FieldT(bv_0jk.ConvertToInt<long>()),
                                        modulus);
    ...
    for (usint i = 1; i < in_lc.size(); ++i) {
        ...
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t k = 0; k < in_lc[0][j].size(); ++k) {
                ...
                LazyMulModGadget<FieldT> g0(pb, in_lc[i][j][k], in_max_value[i][j], FieldT(bv_ijk.ConvertToInt<long>()),
                                            modulus);
                g0.generate_r1cs_constraints();
                g0.generate_r1cs_witness();
                ...
                LazyMulModGadget<FieldT> g1(pb, in_lc[i][j][k], in_max_value[i][j], FieldT(av_ijk.ConvertToInt<long>()),
                                            modulus);
                g1.generate_r1cs_constraints();
                g1.generate_r1cs_witness();
...
void LibsnarkProofSystem::ConstrainFastKeySwitchCore(
    ...
    for (size_t j = 0; j < (*digits)[0].GetNumOfElements(); ++j) {
        ...
        size_t modulus = (*digits)[0].GetElementAtIndex(j).GetModulus().ConvertToInt<long>();
        for (size_t k = 0; k < (*digits)[0].GetElementAtIndex(j).GetLength(); ++k) {
            ...
            PROOFSYSTEM_ASSERT_EQ(mod(pb.lc_val(in_lc[0][j][k]), FieldT(modulus)),
                                  (*digits)[0].GetElementAtIndex(j).GetValues()[k].ConvertToInt<long>());
#endif
            auto av_0jk = av[0].GetElementAtIndex(j).GetValues()[k];
            LazyMulModGadget<FieldT> g1(pb, in_lc[0][j][k], in_max_value[0][j], FieldT(av_0jk.ConvertToInt<long>()),
                                        modulus);
            ...
            PROOFSYSTEM_ASSERT_EQ(mod(pb.lc_val(out_lc[1][j][k]), FieldT(modulus)),
                                  ct1.GetElementAtIndex(j).GetValues()[k].ConvertToInt<long>());
#endif
            ...
            PROOFSYSTEM_ASSERT_EQ(mod(pb.lc_val(out_lc[0][j][k]), FieldT(modulus)),
                                  ct0.GetElementAtIndex(j).GetValues()[k].ConvertToInt<long>());
#endif
        }
    }
